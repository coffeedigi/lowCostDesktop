$(document).ready(function () {
	...
	// 			$(obj).closest(".section-page-question").next().find("button").removeAttr("disabled");
...
// End Button Add to Cart (Buy Now)
// Button Rocket (Start)
var dev = 40;
document.querySelectorAll('.rocket-button').forEach(elem => {
    elem.querySelectorAll('.default, .success > div').forEach(text => {
        ...
        text.querySelectorAll('span').forEach((span, i) => {
            ...
            span.style.setProperty('--d', i * dev + 'ms');
            span.style.setProperty('--ds', text.querySelectorAll('span').length * dev - dev - i * dev + 'ms');
        });
    });
